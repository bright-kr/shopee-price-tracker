"""
Shopee Price Tracker - Bright Data Web Scraper API
----------------------------------------------------
Track prices, availability, and product data from Shopee.

Docs  : https://docs.brightdata.co.kr/scraping-automation/web-scraper-api/overview
Info  : https://brightdata.co.kr/products/insights/price-tracker/shopee
"""
...
# Search for "Shopee" to find the matching Web Scraper (format: gd_xxxxxxxxxx).
...
if not DATASET_ID:
    raise EnvironmentError(
        ...
        "2. Search for 'Shopee' and copy the Web Scraper ID.\n"
...
def track_prices(urls: list[str], **kwargs) -> list[dict]:
    """
    Collect price data for a list of Shopee product URLs.
...
def discover_by_keyword(keyword: str, limit: int = 50, **kwargs) -> list[dict]:
    """
    Discover Shopee products by keyword search.
...
def discover_by_category(category_url: str, limit: int = 100, **kwargs) -> list[dict]:
    """
    Discover all products from a Shopee category page.

    Example:
        results = discover_by_category("https://shopee.com/s?k=headphones", limit=200)
    """
...
if __name__ == "__main__":
    ...
    sample_urls = [
        "https://shopee.com/sample-product-i.12345.67890",
        # Add more Shopee product URLs here
    ]
    results = track_prices(sample_urls)

    output_file = "shopee_prices.json"
    ...
    # cat_results = discover_by_category("https://shopee.com/category/sample", limit=50)
